...
#define APPROX_EQUALS(x, y, tol) bool(std::abs(x - y) < tol)
#include <iostream>
...
namespace SVG {
    ...
    inline Element::BoundingBox Line::get_bbox() {
        using namespace util;
        return { 
          min_or_not_nan(x1(), x2()),
          max_or_not_nan(x1(), x2()),
          min_or_not_nan(y1(), y2()),
          max_or_not_nan(y1(), y2())
        };
    }
    ...
    inline void Element::autoscale(const double margin) {
        ...
        this->get_bbox(bbox);
        double width = std::abs(bbox.x1 - bbox.x2),
            height = std::abs(bbox.y1 - bbox.y2);
    ...
    inline void Element::autoscale(const Margins& margins) {
        ...
        double width = std::abs(bbox.x1 - bbox.x2) + margins.x1 + margins.x2,
            height = std::abs(bbox.y1 - bbox.y2) + margins.y1 + margins.y2,
